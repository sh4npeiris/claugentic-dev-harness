...
def list_expenses(conn: Any, budget_id: int | None = None) -> list[dict[str, Any]]:
    ...
    Reads the expenses, then fills in each row's budget name.
    """
    base = (
        "SELECT id, budget_id, merchant, category, amount_cents, spent_on FROM expenses"
    )
    order = " ORDER BY spent_on DESC, id DESC"
    if budget_id is None:
        cursor = conn.execute(base + order)
    else:
        cursor = conn.execute(base + " WHERE budget_id = ?" + order, (budget_id,))
    rows = rows_as_dicts(cursor)
    for row in rows:
        found = conn.execute(
            "SELECT name FROM budgets WHERE id = ?", (row["budget_id"],)
        ).fetchone()
        row["budget_name"] = found[0] if found else None
    return rows
...
def dashboard_totals(conn: Any) -> dict[str, int]:
    ...
    A store that cannot be read raises out of here. The tile is allowed to be unavailable;
    it is not allowed to say the team spent nothing when the truth is that nobody knows.
    """
    row = conn.execute(
