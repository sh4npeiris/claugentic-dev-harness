...
- **the validator itself is a floating dependency.** `claude plugin validate --strict` runs from `npm install -g @anthropic-ai/claude-code`, resolved fresh on every run (and, in the publish job, installed alongside the credential that writes the branch). That is a **deliberate choice, not an oversight**: a pinned validator goes stale and validates yesterday's manifest rules, which is worse validation than a moving one. The same applies to the `actions/*@vN` pins — mutable first-party majors, an accepted risk, not SHA-pinned.
