...
- **"the workflow is the only publisher"** is a **contract, not a mechanism.** Nothing in this repo's tooling pushes `release` (tests pin that the printed command contains no branch push), but the branch is **not protected** — and on this personal repo it **cannot be protected into one-app-only**: GitHub refuses the Actions app as a ruleset bypass actor on user-owned repos (verified 2026-08-20, API + UI — see *Branch protection* below), so a restrict-push ruleset would block the workflow's own publish. The mechanical alternatives were **declined 2026-08-20**; the contract stays model-upheld (`docs/claugentic-INVARIANTS.md` → *The `release` branch has exactly ONE publisher*);
...
- **the validator itself is a floating dependency.** `claude plugin validate --strict` runs from `npm install -g @anthropic-ai/claude-code`, resolved fresh on every run (and, in the publish job, installed alongside the credential that writes the branch). That is a **deliberate choice, not an oversight**: a pinned validator goes stale and validates yesterday's manifest rules, which is worse validation than a moving one. The same applies to the `actions/*@vN` pins — mutable first-party majors, an accepted risk, not SHA-pinned. **Settled (0041 S10b):** `v0.5.2` (2026-08-18) was the first tag to run this workflow, and `claude plugin validate --strict` ran there **unauthenticated**. If that ever regresses the gates job fails before anything publishes, and the fix is a credential on that step, not a bypass.
...
- **Restrict who can push `release` — verified impossible as specified; declined 2026-08-20.** A ruleset restricting pushes to the GitHub Actions app is not expressible on a personal repo: the app is not an offerable bypass actor (API 422 *"must be part of the ruleset source or owner organization"*; the UI actor picker has no such entry — both verified live), and an active ruleset **without** that bypass blocks the workflow's own `GITHUB_TOKEN` publish push. A trial ruleset was created and deleted. The alternatives — a deploy-key publisher (auth change + a standing secret) or an org move — were declined with it; re-propose only on evidence (a second human gains push rights, or GitHub adds the actor). "One publisher" stays a tooling-upheld contract.
