...
> **Evidence this rule earns its keep** (the diverse-panel trigger now lives in *Principles* above): plan `0003` — a lone reviewer accepted a per-stack drift registry that two diverse lenses cut as self-undermining — and plan `0004` — the honesty pass caught **3 blocking over-claims at Plan**, then confirmed the shipped copy clean at Verify. Details in `DECISIONS.md`.
