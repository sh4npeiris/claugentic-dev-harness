...
> **Diverse critics for a design fork.** When a plan carries a real design fork (a chosen approach a reasonable engineer would contest), run a *diverse* critic set — `yagni-sentinel` **and** an honesty/over-claim pass alongside `plan-reviewer` — not the single reviewer. A lone reviewer can bless a design two independent lenses would reject: in plan `0003`, `plan-reviewer` *accepted* a per-stack drift registry that `yagni-sentinel` + the honesty pass cut as self-undermining (an always-incomplete list re-introduces the very rot it claims to kill). The diverse panel caught it **before** any code.
>
> **The panel applies at Verify (Stage 7) too — not just Plan — for a trust/honesty surface.** When a change touches *what the harness claims to guarantee* (verification scope, `[D]`-vs-`[J]` labels, "proof" vs "attempt" wording, the audit's confidence tags), add the **honesty/over-claim lens to the Verify fan-out** alongside the standards lenses. In plan `0004` (making the audit's verification *universal*) the honesty pass caught **three blocking over-claims at Plan** — a README "arrives with proof" line carried onto the new universal scope, a stale parallel claim, and a collapsed legend that dropped its not-a-guarantee caveat — then **confirmed the shipped copy clean at Verify** (the same lens at both gates). Over-claiming is this repo's #1 risk; for a trust-surface change the honesty lens is not optional.
