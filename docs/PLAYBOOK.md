...
- **The pipelines are code.** The audit, the review panel, and the build loop ship as Workflow scripts (`workflows/verify.js` · `audit.js` · `qa.js` · `build-item.js`). The honest formula: *the skill invokes the script, and the script then runs the fan-out / cross-model re-check / panel mechanically.* If the Workflow tool isn't available in a session, the skill **says so**, runs the prose path, and tags the run "prose-orchestrated" — never claiming the script guarantees. See [`WORKFLOW.md`](WORKFLOW.md) + the skills.
