...
- **Felt / visual craft bar (user-facing work only)** — name the craft bar this surface must clear: the **signature moment** it reaches for, the **per-project design-language voice** it honors (`docs/claugentic-PRODUCT.md` → *Per-project design language*), and the *Aesthetic & motion craft* floor it rides (`docs/claugentic-standards/product-ux.md`). The template forces this to **exist** (`[D]`-presence); `synthesizer-gate` (plan-gate) reviews it's genuinely reasoned (`[J]`); `yagni-sentinel` guards over-build (a **named bar**, never "animate everything"). This forces the craft *question* — it never certifies craft. **Non-user-facing work skips it** (effort-dial — no craft bar on a script/config change).
