...
- **`/claugentic-dev-harness:condense`** — the harness keeps a few managed ledgers (DECISIONS, ROADMAP, and friends) lean by a byte budget; when one gets **near** its budget you'll get a warning, and if it goes **over** the commit stops until it's tidied (both only where `init` wired the commit hook — see *Under the hood* below). `:condense` does that tidy-up *for* you: it classifies every entry first, then proposes a diff you approve before anything is written — nothing is deleted without your say-so, and git history keeps the full record.
...
- **Structure long runs to keep caches warm** — each subagent starts with a fresh prompt cache, and the platform's prompt-caching reuses a stable prefix (its cached entries expire after a short idle window). So a long multi-agent run goes faster when it's structured to reuse that warm prefix — batch a specialist's work rather than scattering it across long gaps, and take task breaks at natural boundaries. This is a *speed* lever, not a correctness one; the harness's fan-out and slice-complete grain already tend to produce it.

**When the commit-time checks run — at commit, not while you edit.** Two run there, chained into one hook **wherever `init` wired that hook** (a repo that kept its own architecture tree has no commit hook at all — there both checks are on-demand), and the same pair also runs when a `git merge` commits cleanly: the architecture-tree check and the **doc-budget check** (which measures your ledgers against the caps `init` seeded in `.claude/claugentic-doc-budgets.json` — a ledger near its cap prints a warning and lets the commit through; one over it stops the commit until you condense — that is the nudge `/claugentic-dev-harness:condense` above answers). The rest of this section is about the tree check. The harness keeps an architecture tree (`docs/claugentic-ARCHITECTURE_TREE.md`): a one-line-per-file index the agents use to find the right file without reading the whole codebase. When you add or move a source file you update that tree, and a **pre-commit gate** checks it is current — it runs at **`git commit` time, not live while you edit.** That's a deliberate design, not a limitation: a commit-time check is **deterministic** (the same result on every machine with a working Python — without one the gate skips loudly and the commit passes) and **portable** (no editor- or session-specific tooling to install), so the tree can't quietly drift out of step with the code. If you'd rather catch a stale tree the moment you save, you can wire **your own** optional editor/`PostToolUse` hook to run the same check in-session — that's yours to add, not something the harness ships.
