...
The harness has three **finders** that surface candidate work — `audit` (engineering, vs the `docs/claugentic-standards/` modules), `product` gap-mode (vs the spec), and `doctor` (harness-health) — and they share **one** lifecycle: **`FIND → SELECT → PLAN → OFFER-BUILD → BUILD`**. The shared pipeline exists so effort isn't spent elaborating items the user never wanted: nothing is planned, spec'd, or written to the backlog until the user has **chosen** it. **SELECT** (the scope gate) is detailed below; **PLAN / OFFER-BUILD / BUILD** are the continuing contract wired by the build pipeline (sketched at the end).
...
**PLAN / OFFER-BUILD / BUILD (the continuing contract — wired by the build pipeline).** A SELECTed (committed) item gets its **full plan preemptively** via Stage 2 — the architect-pass producing problem · architecture-fit · approach · decomposition · the finding's evidence — not a token stub. **OFFER-BUILD** then asks *"build these now, or leave them in the backlog?"* (default: leave). **BUILD** works selected items **one at a time**, each starting from its pre-made plan, with the **deep per-slice Spec → Approve → Implement → Verify run just-in-time** as build reaches that slice (step-level pruning happens at plan-review/approve, so no slice is deep-spec'd before its turn). **Commitment, not capture, triggers the plan:** selecting an item *is* committing it *is* the Stage-2 trigger — a raw Bug / Later one-liner stays planless until selected, and a trivial / mechanical item skips the backlog entirely and is just done.
