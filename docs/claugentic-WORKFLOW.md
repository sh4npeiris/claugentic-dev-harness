...
- **Render from the selected subset only — zero engine change.** Filter the engine's `result.items` to the ticked subset, then render the fence from it: `renderBacklogFence({ ...result, items: selected })`. `renderBacklogFence` already filters `result.items` by tier internally (see `engine/audit.js`), so passing a pre-filtered `items` is all it takes — **no engine edit, and no `selectedIds` argument** (SKILL-side filtering is strictly simpler).
- **★ INVARIANT — coverage / run-report stay full-scope.** The `lensCoverage` and `verification` / run-report fields handed to the renderer remain the **unfiltered, full-audit** values. The fence then reports honestly: *"we looked at all of this; you chose to act on these."* **Never recompute coverage over the selected subset** — that would falsely claim "every lens spoke" about only the kept findings. Because the skill filters **only** `items` and passes the rest of `result` through unchanged, this holds automatically — but it is stated as an invariant so a future edit doesn't also filter `lensCoverage` / `verification`. (This is the verify-side honesty sibling of the audit cell-coverage fix.)

**Three candidate sources feed SELECT** (named here at contract level): the `harness-audit:backlog` candidates, the `harness-product:backlog` candidates, and the durable **Bugs** section (planless one-liners; selecting one **commits** it). The Bugs-as-source reader is wired in the audit/product retrofit.
