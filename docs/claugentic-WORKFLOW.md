...
**PLAN — preemptive, per committed item (architecture-first, not a stub).** When an item is **committed** (SELECTed to act on), its **full plan file** is produced via **Stage 2** — the architect-pass: **2a** plan-mode draft → **2b** advisory panel (substantial work only) → **2c** incorporate → the **Stage-3 `plan-reviewer` gate** — structured by `TEMPLATE.md`: problem · **Architecture & holistic fit** · approach · decomposition · the finding's evidence (`file:line`), plus a one-line `ROADMAP.md` entry. The plan is **specialist-produced groundwork** so build never re-derives context from scratch — but it is **reviewed groundwork (`plan-reviewer`-gated), never a guarantee of quality**; the whole step is **model-upheld** (the skill runs the architect-pass; nothing mechanical forces a plan).
