...
- **Reviews run on the most-capable model (Opus) — the SAME model as the builder; the independence is STRUCTURAL, not model-based.** The four gate/refute roles — `plan-reviewer` · `architect-reviewer` · `honesty-reviewer` · `finding-verifier` — run on Opus (script runs pin `model: MODELS.judge` = `opus` in the engine's `MODELS` block; prose runs use the agents' `model: opus` frontmatter). The real safeguard is the **clean-context input contract**: each judge works only from `{claim, file:line, …}` and **never sees the builder's rationale or transcript**, so it cannot rubber-stamp the builder's reasoning. This is deliberately **NOT model-independent review** — the judge and builder are the same model family, so blind spots correlate — and the harness says so: each judge opens with a `RUNNING AS: <model family>` self-report and the run reports the actual model relationship (normally **same-model**, since the builder typically also runs Opus). **Honest claim:** a skeptical, clean-context second pass on the most-capable model — a reduction of rubber-stamping risk, never a guarantee. *(The engine keeps the `RUNNING AS` / `sameModelTag` machinery as an honest per-run reporter; it no longer engineers cross-model de-correlation. We pin model **tiers** — `opus`/`sonnet`/`haiku` — which auto-resolve to the current model, never a frozen version.)*
