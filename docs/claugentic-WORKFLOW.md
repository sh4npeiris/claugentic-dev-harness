...
| 2 | **Draft plan** | FRAME | orchestrator / `Plan` | `.claude/plans/NNNN-<slug>.md` (Problem · Approach · **Architecture & holistic fit** · Risks · Test strategy · slices — Review & Spec filled later), **sliced into ≤1-session units**. For *substantial* work the draft **MUST** fill the **Architecture & holistic fit** section (the forcing function — see below); trivial/lightweight changes keep the lightweight path and may give it a one-liner or skip it |
