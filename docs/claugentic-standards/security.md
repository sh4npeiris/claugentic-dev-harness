...
- **Good looks like —** The CI/CD pipeline is treated as a production system with its own authorization model. Workflow-level permissions are **read-only by default**, with the write/publish grant scoped to the single job that publishes; the job that runs untrusted work — the test suite, a third-party install, anything reachable from a PR — never holds the credential that ships. What makes a commit *releasable* is checked, not assumed: the published ref is verified to be an **ancestor of the trunk**, the tag matches the manifest version, and the publish job `needs:` the gate job. Publishing runs the **one tested build path**, never a re-implementation in YAML. Floating actions/installers inside the credentialed job are pinned — or the non-pinning is a **recorded accepted risk** with its reason.
...
- **Confidence —** `mixed`
- **Tradeoff (plain English) —** The release pipeline is the one place where a mistake ships to every user at once, and it usually runs with more permission than any application code. Scoping its authorization costs two lines of YAML; getting it wrong means the job that runs code from the internet can also rewrite what your users install. Over-tightening breaks legitimate publishes — so scope *by job*, don't strip grants wholesale.
- **Incident that motivated this (delete this rule once its cause is gone) —** Plan 0041 Slice 2 (2026-08-12): the **spec itself** prescribed `permissions: contents: write` at *workflow* level and the implementation matched it faithfully — arming the gates job (full suite + an unpinned global npm install) with a write-capable token. The security lens refuted the **spec**, not the diff (amended to workflow-level `contents: read`, `write` on `publish` alone). Two further findings sat in the same hole: the pre-existing base-ancestry guard was *reverse* inclusion, so a never-merged descendant could be tagged and published (fixed with `git merge-base --is-ancestor "$GITHUB_SHA" refs/remotes/origin/main`), and a floating `npm install -g` ran inside the credentialed publish job, documented nowhere. The verdict's own words: *"dimension the spec missed: tag-commit authorization — no gate covered it."*
- **Sources —** GitHub Actions security-hardening guidance (least-privilege `GITHUB_TOKEN`, job-scoped `permissions`, untrusted input); OpenSSF **SLSA** build-provenance levels (build-platform integrity; a single, scripted build path); cross-ref *Dependency & supply-chain hygiene* above and `reliability-resilience`.
