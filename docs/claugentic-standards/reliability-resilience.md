...
- **Confidence —** `mixed` — the released-shape diff and the remedy execution are mechanical; which populations must be supported is a judgment.
- **Tradeoff (plain English) —** Your working tree is the cheapest possible model of "what the reader has", and it is wrong the moment your branch diverges from the last release — which is always. Diffing the tags costs one command per supported version. Skipping it buys a repair path that cannot fire for anyone who needs it, plus advice that, followed literally, is worse than the gap it repairs: it breaks something that was working.
- **Incident that motivated this (delete this rule once its cause is gone) —** Plan 0041 Slice 7 (2026-08-16). An installer learned to chain a second gate into an adopter's existing pre-commit wrapper, and to **refresh** any wrapper "whose run logic is exactly the prior shipped shape". Nobody diffed the tags: **no release had ever shipped that shape** (three published versions carry a 7-line wrapper with no `run_gate` function — independently re-verified, `grep -c run_gate` = 0 at all three tags; the compared shape existed only on `main`, and even there differed by one reworded executable line). So **100 % of the population the branch named** fell through to the never-clobber branch — whose printed one-line fix was then run through a real `sh` against the actual released wrapper: the inserted line's exit status was read by the following `[ $? -eq 0 ] && exit 0`, turning a blocked commit (exit 1) into a **pass** (exit 0) with the breach output discarded. The remedy **disarmed the gate the adopter already had.** A third state (a wrapper equal to the *current* template) was unenumerated, so a settled re-run told the user they had edited the file and to add a line already present — applying which chains the gate twice. One root cause behind all four: the reader's baseline was assumed to be the author's checkout.
- **Sources —** cross-ref *Silent disarm* above (a remedy that lowers a gate's exit status is that dimension's failure mode, reached from the upgrade path) and *Idempotency & safe retry* below (the already-current branch is what makes a re-run a no-op); `docs-traceability.md` → *Reach, not residence* (the **location** axis of the same "measure in the reader's world" bar); Semantic Versioning 2.0.0; Nygard, *Release It!* (upgrade/rollback safety).
