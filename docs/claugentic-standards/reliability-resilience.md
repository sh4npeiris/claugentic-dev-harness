...
- **Auditor checks —** `[J]` each non-critical external dependency has a circuit-breaker library or manual tripped-state guard · `[J]` the degraded-mode response is intentional and documented.

## Thread safety & concurrency hazards

- **Auditor checks —** `[J]` shared mutable state identified (class-level variables, singletons, module globals) and access serialized, or the object documented thread-local · `[J]` async code free of unguarded concurrent writes to shared collections · `[J]` channels/queues bounded or carrying explicit backpressure.
