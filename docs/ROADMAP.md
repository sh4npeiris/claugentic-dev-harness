...
| 7 | **Curate the standards catalog to support the 3-rung depth dial** — verify the catalog's coverage and per-module depth actually yield effective, valuable results across `focused` / `deep` (and the coming `thorough`) reads; fill gaps the depth dial now exercises. | The depth dial only pays off if each lens has enough authored bar to dig into; a follow-on to plan 0004 (deliberately not in that slice). | NEXT |
