...
This repo is **`claugentic-dev-harness`** — a **Claude Code plugin**, not a conventional
...
- **Scaffold itself** (`/claugentic-dev-harness:init`) — copy in the standards + workflow docs
...
- **Audit that repo** (`/claugentic-dev-harness:audit`) — read the codebase, explain in plain English
...
itself**: `/claugentic-dev-harness:audit` ran on this very repo to produce a backlog, which was then
...
- `skills/` — the two installable commands (`init`, `audit`), each a
  `SKILL.md`.
...
- **What's *not* there (and is fine for this kind of repo):** **no test runner, no
  ...
  repo that *does* have app code brings its own test/lint tooling, which `/claugentic-dev-harness:init`
...
## Audit backlog  _(generated by /claugentic-dev-harness:audit · do not edit inside the fence — re-run to refresh)_
...
| the `audit` skill's `thorough` level — a second adversarial-verify pass + a completeness-critic over the audit findings (cut from S2b-i as YAGNI for a model-asserted v1 backlog). | LATER |
