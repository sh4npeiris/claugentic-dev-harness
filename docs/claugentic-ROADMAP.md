...
- **`harness`** — extend the agent-namespace presence-check (or a sibling gate) to scan `skills/**/SKILL.md` **and `.claude/plans/TEMPLATE.md`** for a stranded `claugentic-dev-harness:<old-role>` literal after a roster rename. Today `agent-namespace.test.mjs` pins only `engine/*.js`, so a rename can strand a LIVE spawn directive in a SKILL or the (actively-copied) plan template with all tests green — exactly the silent-miss class the 0026 §C3a review caught by hand (the `synthesizer-gate` rename had stranded `plan-reviewer` in `TEMPLATE.md:29/:54`). A presence-scan over those live-directive surfaces would make it mechanical. (Stage-9 harvest, 0026 §C3a/§C5b.)
