...
- `.claude/agents/architect-reviewer.md` — owns the Verify gate (Stage 7): audits the diff against in-scope `docs/standards/` modules in **solo** mode (small changes) or **synthesizes** lens-reviewer + yagni-sentinel findings in **fan-out** mode; read-only. A gate/refute judge — spawned with the `fable` model override (cross-model: a different model family than the builder, a shared-blind-spot-risk reduction; opens `RUNNING AS:`; same-model runs tagged); frontmatter stays `model: opus` (the honest fallback).
...
- `.claude/agents/honesty-reviewer.md` — the harness's **claims / over-claim** lens: refutes **copy** (not code), flagging text that launders model-or-human-upheld judgment into apparent mechanical fact (the verb discipline · `[D]`/`[J]` label integrity · dimension-scoped success claims · `cross-model ≠ independent`); the bar is **embedded in the prompt** (no standards module — promote when a 2nd consumer appears). Part of the diverse panel at Plan + Verify on trust/honesty surfaces; read-only. A gate/refute judge — spawned with the `fable` model override (cross-model: a different model family than the builder, a shared-blind-spot-risk reduction; opens `RUNNING AS:`; same-model runs tagged); frontmatter stays `model: opus` (the honest fallback). A reduction of false confidence, not a deterministic gate (does **not** mechanically cross-check).
...
- `skills/audit/SKILL.md` — Understand + Audit + Backlog. Phase 1 = inline overview + audit-plan; Phase 2 = one uniform **FIND → PRUNE → VERIFY → surface** pipeline — **one thoroughness slider** (`quick`/`standard` auto-dialed from Phase 1, `thorough` named-only) each stage responds to: FIND digs by depth (`focused`/`deep`/`exhaustive`) and at `thorough` *also* runs a `blindspot-reviewer` cross-cutting sweep; PRUNE is synthesis self-review, *plus* a `thorough`-only adversarial `yagni-sentinel` sweep; **VERIFY is flat** — an **attempt to re-check every surfaced finding (all tiers) via `finding-verifier`** at every notch. `lens-reviewer` fan-out (audit-scope mode) audits each `(module×dir)` cell **once**, then dedup; deterministic resume + one shared budget cap; Phase 3 = tiered/tagged backlog into the `harness-audit:backlog` fence with **one verification tag per item** (+ the not-a-guarantee legend caveat) + the **"architecturally sound" terminal signal** when Tier 1/2 are empty; the close-out "how to start" line hands off to `/claugentic-dev-harness:build` as the per-item go-button.
- `skills/build/SKILL.md` — the backlog **go-button**: a thin orchestration layer that drives `harness-audit:backlog` items through the `docs/WORKFLOW.md` pipeline (plan → review → spec → **your approval** → implement → verify → land) in **checkpoint mode** (the only live mode), pausing per item at exactly three decisions (the spec before any code · before land · before any irreversible action). **Two paths:** a single named item = the fast path (no triage); **the full-backlog loop** = multi-item triage (pick items and/or tier shortcuts → confirm the ordered worklist → "start now?") then work it item-by-item, each through the per-item engine, with a **scoped re-audit over the touched `(module×dir)` cells** after each item (honest scope — cross-file fallout owned by the closing full audit, no "dependents" claim) that **pauses to re-triage on any new Tier-1/2** else auto-continues, ending in **one `standard` full audit** → the verbatim "sound on the audited dimensions" terminal signal (or the remainder surfaced for a final triage). The **resume contract derives the worklist** from the backlog fence + in-flight plan files + archived plans (no new state store; order re-confirmed). Spawns the existing pipeline subagents (`plan-reviewer` + the diverse panel, `implementer-architect`, `architect-reviewer` + lenses, the audit's `lens-reviewer`/`finding-verifier` for the re-audit); tag→discipline + the Verify dial + the DoD point at WORKFLOW (no restatement). `autopilot` is a named mode whose ONLY behavior is an honest decline naming Roadmap #4+#5 (no scaffolding). Guardrails: the irreversible hard-stop set (incl. push-to-`main`), no-invented-scope, the honesty register (completed-beat narration, never an ETA).
