...
- `skills/build/SKILL.md` — the per-item **go-button**: a thin orchestration layer that drives ONE named `harness-audit:backlog` item through the `docs/WORKFLOW.md` pipeline (plan → review → spec → **your approval** → implement → verify → land) in **checkpoint mode** (the only live mode), pausing at exactly three decisions (the spec before any code · before land · before any irreversible action). Spawns the existing pipeline subagents (`plan-reviewer` + the diverse panel, `implementer-architect`, `architect-reviewer` + lenses); tag→discipline + the Verify dial + the DoD point at WORKFLOW (no restatement). `autopilot` is a named mode whose ONLY behavior is an honest decline naming Roadmap #4+#5 (no scaffolding). Guardrails: the irreversible hard-stop set (incl. push-to-`main`), no-invented-scope, the honesty register. One item at a time (the full-backlog loop is Slice 2).
