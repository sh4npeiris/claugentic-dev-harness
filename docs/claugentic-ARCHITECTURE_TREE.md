...
- `docs/diagrams/*.{excalidraw,png}` — README diagrams (Excalidraw source + PNG) that **ship** with the plugin: `harness-usage-flow` (the *Commands* command-map) and `harness-journey` (the *Under the hood* pipeline). Both drawn at v0.4.1; the README carries their as-of notes.
