...
- `docs/diagrams/*.{excalidraw,png}` — README diagrams: `harness-usage-flow` (the *Commands* map) and `harness-journey` (the *Under the hood* pipeline). The **`.png` ship** (the README embeds them); the **`.excalidraw` edit sources are dev-only** (stripped 2026-08-20). Both drawn at v0.4.1; the README carries their as-of corrections and a redraw is pending.
