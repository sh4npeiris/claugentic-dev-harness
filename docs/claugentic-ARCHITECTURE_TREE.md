...
- `docs/claugentic-DECISIONS.md` — open before re-litigating a past choice: the maintainer guide of standing decisions by area (honesty, gates, judges, audit, build, workflow, plugin) + Readiness footer. Condensed, not append-only.
...
- `scripts/check_doc_budgets.py` — open to change the ledger byte-budget gate: flags a managed ledger over its `DOC_BUDGETS` cap (CLAUDE 6K · DECISIONS 60K · ROADMAP 12K · INVARIANTS 20K) + a ≥90% WARN; independent fail-loud reads. Not hook-wired.
