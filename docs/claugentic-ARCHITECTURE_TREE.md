...
- `tests/test_build_release.py` — release-builder tests: ship-vs-strip split + six-class no-op, stale-base/version-increase/drop-check guards, the S10 `--bump` writer (`TestBumpManifests`: targeted dual-manifest replace · both-or-neither · idempotent) + `TestApplyBumpOrchestration` (one-command flow: exhaustive abort · no-dangling-tag · retry · gated-command-printed-not-run); hermetic. *(Out of `INCLUDE_GLOBS`.)*
