...
- `pyproject.toml` — open when changing test deps, discovery, or lint scope: the PEP-735 `test` dependency group CI installs from + pytest config (`testpaths`, `integration` marker) + ruff, which excludes the deliberately-defective fixture dirs and nothing else, plus one reasoned per-file rule ignore.
...
- `docs/claugentic-decisions/*.md` — the ledger's per-topic shards; `claugentic-DECISIONS.md` is the roster and the router (pinned both ways by `tests/test_decisions_index_agreement.py`). Reach them VIA the index, never linked directly.
...
- `docs/RELEASE_CHECKLIST.md` — the CI-publishes ritual: what a release actually reaches (plugin-resident vs copy-on-init), prepare with `build_release.py --apply --bump <version>`, push one tag, the workflow gates + publishes; red-run recovery, and the honest-scope split (what CI guarantees vs the `[J]` eval-drift/`range-diff`/branch-protection halves).
- `docs/claugentic-PLAYBOOK.md` — open for the non-engineer's driving guide: the one-minute model, your three leverage points (incl. approving a spec), why it's trustworthy, how to start work, using the audit, `:doctor`/`:condense`, how the pipelines run, Under the hood.
- `docs/claugentic-PRODUCT.md` — open for durable product/UX context (`product-designer` Stage-1 output): user + design language, the per-project design-language (anti-sameness) record, and the Build-mode brief (JTBD · which states must exist · what-good-feels-like · UX failure modes). Mechanics + copy stay in `skills/build/SKILL.md`.
...
- `.claude/agents/synthesizer-gate.md` — the GATE (integrate→verdict→loop) at three altitudes: plan-gate (Stage-3, EDITS the plan's Review), verify-verdict (Stage-7, audits test-first independence), audit-synthesis (tiers findings). Clean-context, no model pin; spawned from `engine/{verify,audit}.js`.
...
- `.claude/agents/finding-verifier.md` — the audit's refute counterpart: given ONE finding (claim + `file:line`, never the finder's rationale) tries to refute it → Verified/Refuted/Unconfirmed; per-finding after the prune. Read-only.
- `.claude/agents/honesty-reviewer.md` — the over-claim lens: refutes COPY not code (verb discipline · `[D]`/`[J]` integrity · `clean-context ≠ independent`); on the diverse panel at Plan/Verify on trust surfaces. Read-only.
...
- `.githooks/pre-commit` — open when changing the commit gate: THE one gate chain, wired via `core.hooksPath=.githooks`; `run_gate <script> [args]` runs each gate once per commit (stdout captured = silent clean pass, stderr flows through, exit 1 aborts); broken git skips silently, no working Python / absent gate script skip loudly. Template twin in `skills/init/SKILL.md` (parity-pinned).
- `.githooks/pre-merge-commit` — the merge entry point: git fires THIS (never `pre-commit`) for a conflict-free `git merge`, so without it neither chained gate sees a merge result. One `exec` of the sibling wrapper — delegates, never duplicates the chain.
...
- `engine/qa.js` — open to change runtime verification: boot + flow-driving (`boot-only` / `full`). A boot agent probes `appUrl` on a bounded `readinessPlan` and ALWAYS tears down via `teardownPlan` in a finally (port freed on success, boot failure and mid-run error alike); full mode drives one `runtime-qa` per criterion (`e2e` = real browser, `api` = HTTP, `manual` never driven) → re-checked by finding-verifier.
...
- `scripts/check_standards_anchors.py` — the inbound-section-citation resolver (harness-self run-gate; `--targets` defaults to `docs/claugentic-standards/`, so cut slices reuse it): resolves every anchor/arrow/quoted citation across `git ls-files` — wrap-normalized, both arrow glyphs — against the target's LIVE headings; exit 1 on a dead one. A self-test canary re-proves the scanner EVERY run, so a clean zero is never a silently-broken pattern.
...
- `tests/test_precommit_wrapper.py` — runs `.githooks/pre-commit` through real `sh` (fake gate + a REAL-gate `git commit` e2e): stub-beside-working Python still gates, absent Python/gate script skip loud, broken git skips silently, stdout captured, stderr through, red aborts; plus init-template parity, the husky rules, the floor pin, and the `pre-merge-commit` pair (a merge refused; allowed with that hook gone). *(Out of `INCLUDE_GLOBS`.)*
...
- `tests/test_check_standards_anchors.py` — tests for the citation resolver, in two halves: the forms it must RESOLVE (both arrows, quoted, backticked, undelimited, wrapped, bare-basename, anchor slug, word-boundary partials) and the breaks it must REDDEN (cut/renamed heading, fenced pseudo-heading, prefix-only anchor, disarmed self-test probe) — plus the live sweep pin. Hermetic tmp corpora; no git. *(Out of `INCLUDE_GLOBS`.)*
...
The harness's runnable targets: `fixture-app/` for runtime verification (`engine/qa.js`), `fixture-defects/` — the seeded-defect baseline the audit re-takes as a drift detector — and `fixture-build/`, the task an `implementer` builds so the BUILD path's output can be measured; **both exams' flaws are planted on purpose — never 'fix' them (that disarms the exam)**. *(All `eval/` is OUT of `INCLUDE_GLOBS` — listed for the map, invisible to the gate by design; see INVARIANTS → eval source stays out of gate scope. Deps documented-not-installed; `pyproject testpaths=["tests"]` keeps bare pytest out of `eval/`.)*
...
**Do not revert the `fixture-build/` entries below to trap-level detail.** Same rule, second exam: an entry says what the file **is**, never which mistake it plants, catches or carries — and that binds hardest on `calibration/`, whose mutant directories are listed as ONE line on purpose (a per-file entry would name the trap each one flips). `tests/test_eval_key_containment.py` pins the **id** half mechanically and this note is the paraphrase half's only guard; a build-eval answer belongs in `TRAP_MANIFEST.md` or, for recorded runs, `eval/BUILD_ENTRIES.md`.
...
- `eval/fixture-build/TASK_SPEC.md` — builder-visible: the spendlog brief (stdlib+sqlite expense tracker), R1-R9, the pinned public surface table and the two-table schema. Byte-identical in both arms.
- `eval/fixture-build/plan-slice.md` — builder-visible: the frozen approved plan slice copied into each run worktree's `.claude/plans/`, so the shipped `implementer` contract is used UNMODIFIED, with the run's scoping (writes confined to `out/`; the architecture tree and this checkout's gate scripts out of scope) delivered through the contract's own plan-file channel.
- `eval/fixture-build/TRAP_MANIFEST.md` — the build eval's answer key: ten rows (`id · module · spec req · harm line · check · tag`), the admissibility rule, the denylist rationale and its own canary. MUST NOT be read during a run.
- `eval/fixture-build/checks/` — the instrument: `eval/fixture-build/checks/test_heldout.py` (held-out suite), `eval/fixture-build/checks/fakes.py` (fault-injection fakes), `eval/fixture-build/checks/mutation_probe.py` (removal-and-re-run prober), `eval/fixture-build/checks/run_sweep.py` (entry point; `sweep`/`judge-pack`). Facts, never a score; **what each decides is `TRAP_MANIFEST.md`'s business, never this index.**
- `eval/fixture-build/calibration/reference/` — the clean reference build: one module per row of the brief's pinned surface table, plus the project's own test file. Every probe must come back clear on it, and the mutants are overlays of it.
- `eval/fixture-build/calibration/mutants/` — overlay directories, at least one per manifest row (a directory may carry a `.variant` suffix for a second shape of the same row), each replacing the file(s) it needs; listed as one line on purpose (see the note above). Read `TRAP_MANIFEST.md` for what they are, never this index.
- `eval/fixture-build/calibration/run_calibration.py` — the instrument's own verification: sweeps the reference and every mutant, refusing the exam unless the reference is clean and each mutant flips its own probe and nothing else.
- `eval/BUILD_BASELINE.md` — open before a build-eval run: the procedure, the pre-registered decision rule, the standing rules and the `H`/threshold pin. Carries NO answers by design (the id half of that is scanned; the paraphrase half is model-upheld).
- `eval/BUILD_ENTRIES.md` — the build eval's results ledger, append-only, newest first, human-stamped; answer-bearing (per-run trap tables), which is why it is not the file a runner opens for procedure.
...
- `tests/test_eval_key_containment.py` — the answer-key containment pin, now for BOTH exams: no tracked file outside each exam's justified allowlist names one of its ids (each vocabulary derived from its own manifest, corpus from `git ls-files` with a floor); also pins the two do-not-revert notes. *(Out of `INCLUDE_GLOBS`.)*
- `tests/test_eval_trap_manifest.py` — the build fixture's integrity guard: the trap table parses (10 rows / 2-per-module), every mechanical row resolves to a real callable, judged rows state a rule, the no-coaching lint over both builder-visible artifacts, the `checks/` read-scope and catalog-free scans, and the `H` pin against the real test count. *(Out of `INCLUDE_GLOBS`.)*
