...
- `scripts/build_release.py` — open to change the release builder: `DEV_ONLY_PATH_CLASSES` is the single authored `path -> recreate-class` manifest (six classes); `classify()`/`is_dev_only()`/`recreate_class()` derive from it (`DEV_ONLY_FILES` = a keys alias); `--apply` rebuilds a local `release` branch (refuses a stale base via `_dropped_merges`), `--dry-run` prints the split.
...
- `scripts/check_shipped_content.py` — shipped-content scanner gate: `import build_release` for the ship set + `recreate_class` manifest (dangle/recreated/self-gate/config partitions DERIVED off the classes, not hand-lists — 0034 S2); Pass C non-ASCII engine `*.js` + Pass B stranded `claugentic-dev-harness:<token>` (VALID=FS agents∪skills∪`{update}`) + A.a dangling stripped-path refs HARD (exit 1), A.b gate-mention WARN; fail-loud. Harness-self.
...
- `tests/test_build_release.py` — tests for the release builder: `TestClassify` pins the ship-vs-strip split, `TestManifestMigration` pins the frozenset->`DEV_ONLY_PATH_CLASSES` dict no-op (shipped set byte-identical + exhaustive six-class partition + `recreate_class`), `TestBaseAncestryGuard` pins `_dropped_merges` (empty/two-SHAs/missing-ref); hermetic via monkeypatched `_git`. *(Out of `INCLUDE_GLOBS`.)*
