...
- `scripts/build_release.py` — release builder: `DEV_ONLY_PATH_CLASSES` = the authored `path -> recreate-class` manifest (six classes); `classify()`/`is_dev_only()`/`recreate_class()` derive from it; `--apply` rebuilds a local `release` branch behind three fail-loud preconditions — `_missing_upstream_commits` (stale base), `_version_increase_error` (tag-anchored semver), `_dropped_shipped_paths` (subset drop-check); `--dry-run` prints the split.
...
- `scripts/check_shipped_content.py` — shipped-content scanner gate: `import build_release` for the ship set + `recreate_class` manifest (partitions DERIVED off classes — 0034 S2); HARD passes (exit 1) C non-ASCII `*.js` · B stranded `claugentic-dev-harness:<token>` · A.a dangling stripped-path refs · D closure `NEEDS ⊆ HAS` (init-seed/init-gen/recreate-on-demand/self-gate — 0034 S3); A.b gate-mention WARN; fail-loud. Harness-self.
