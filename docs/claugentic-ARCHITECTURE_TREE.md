...
- `scripts/check_shipped_content.py` — shipped-content scanner gate: `import build_release` for the ship set; Pass C non-ASCII engine `*.js` (codepoint > U+007F) + Pass B namespace (stranded `claugentic-dev-harness:<token>`, VALID = FS-derived agents ∪ skills ∪ `{update}`) + Pass A.a dangling stripped-path refs are HARD (exit 1), Pass A.b uncaveated gate-mention is WARN; fail-loud on git/read. Harness-self (DEV_ONLY).
