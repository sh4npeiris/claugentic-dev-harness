...
- `tests/test_adopter_pointer_integrity.py` — pointer-reach pins (0041 S9): no role file points at `CLAUDE.md` → *Honesty positioning* (a heading in neither repo), WORKFLOW's adopter note precedes `## 0.`, and the upstream channel URL (read from the manifest) is named there exactly once. *(Out of `INCLUDE_GLOBS`.)*
