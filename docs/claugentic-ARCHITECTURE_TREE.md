...
- `docs/claugentic-CHARTER.md` — the OPTIONAL engineering charter (adopter-local, `init` step-7 create-if-absent from the `_CHARTER.md` seed; never refreshed): a living per-work-type methodology record (record/apply/adapt/grow, update-not-append). **Absent in THIS repo** — the harness follows its own default grain (absent ≡ current behavior).
- `docs/RELEASE_CHECKLIST.md` — thin wrapper around the ONE release command `build_release.py --apply --bump <version>` (writes both manifests + builds + validates, STOPS + prints the gated `git tag && push --force-with-lease && push tag`); keeps the model-upheld `[J]` steps (eval drift-check, `range-diff`, the human push) + the one-time `v0.3.1` bootstrap tag.
...
- `docs/diagrams/*.{excalidraw,png}` — README diagrams (Excalidraw source + rendered PNG), repo-level assets NOT shipped to adopters: `harness-usage-flow` (the command map in README's *Commands* section — how init/audit/product/build/doctor/condense connect) and `harness-journey` (the full pipeline in *Under the hood* — the four beats, gates, methodology charter, Stage-9 learning loop).
...
- `docs/claugentic-PLAN_TEMPLATE.md` — open when starting a plan: the managed plan-file contract template (Problem/Goals/Approach/**Architecture-&-holistic-fit** incl. the user-facing **felt/visual craft-bar** forcing-function/Affected-files/Research/Risks/Tests/Decomposition/Review/Spec + Status block). Adopters copy one per plan into their own `.claude/plans/`.
...
- `docs/claugentic-_CHARTER.md` — the SHIPPED pristine adopter CHARTER seed (one-time-seed kind): `init` step 7 copies it → `docs/claugentic-CHARTER.md`, underscore stripped, create-if-absent. Optional living per-work-type methodology record; ships nearly-empty (prose header + commented examples), no harness content.
- `docs/claugentic-PRODUCT_SPEC.md` — the harness's own filled spec (dogfood example): who-for/job/promise, the honest-disclosure invariant, the six command-features (4 core + doctor/condense) + frozen-schema criteria PS-1..PS-7 (all `manual`). User-owned, never stamped/auto-refreshed.
...
- `.claude/agents/synthesizer-gate.md` — the GATE (integrate→verdict→loop) at three altitudes: plan-gate (Stage-3, EDITS the plan's Review section), verify-verdict (Stage-7, solo or synthesizes the panel; consults the methodology toolbox + charter record, conditionally audits test-first-when-chosen independence), audit-synthesis (tiers audit findings); clean-context opus, opens `RUNNING AS:`; pinned in `engine/verify.js` + `engine/audit.js`.
- `.claude/agents/implementer.md` — Stage-6 builder (native implement): implements one approved, spec'd slice to standard in an isolated worktree; lands code + tests + docs, no debt; upholds the CLAUDE.md principles + in-scope standards; consults the methodology toolbox + charter record (apply/record/adapt/grow) and runs red-first when test-first is chosen (independent test-author, greens-without-editing).
...
- `skills/init/SKILL.md` — open to change adoption/scaffolding: the 9-step never-clobber upsert (managed docs, tree+globs, pre-commit hook, plugin self-ref, ROADMAP/DECISIONS/CHARTER seeds). Two modes — Shared (committed, default) vs Solo/local-only (kept fully local; git stays clean).
...
- `scripts/build_release.py` — SINGLE release entry point `--apply --bump <version>` (S10): six-class `DEV_ONLY_PATH_CLASSES` manifest drives `classify`; the one flow runs preconditions (stale-base · `_bump_manifests` targeted dual-manifest write · version-increase · drop-check) → build → `_validate_built_tree` → STOP + print the gated `_gated_publish_command` (tag-at-publish + `--force-with-lease`, never run in-build).
...
- `scripts/check_shipped_content.py` — shipped-content scanner gate: `import build_release` for the ship set + `recreate_class` manifest (partitions DERIVED off classes — S2); scans the dev checkout, or `--root <tree>` a built/stripped worktree from the dev checkout (P0-2, S5); HARD passes (exit 1) C non-ASCII `*.js` · B stranded token · A.a dangling refs · D closure `NEEDS ⊆ HAS` (S3); A.b gate-mention WARN; fail-loud. Harness-self.
- `scripts/claugentic-session-advisor.py` — open to change the SessionStart advisor (not a gate): derives ONE "where am I / what's next" line; size-capped JSON, agent-facing `additionalContext` ONLY on the resume branch (nudges = `systemMessage` only); `CLAUDE_HARNESS_ADVISOR=off` mutes; fail-safe exit 0.
...
- `tests/test_build_release.py` — release-builder tests: ship-vs-strip split + six-class no-op, stale-base/version-increase/drop-check guards, the S10 `--bump` writer (`TestBumpManifests`: dual-manifest replace · both-or-neither · idempotent) + `TestApplyBumpOrchestration` (one-command flow: abort · no-dangling-tag · retry · gated-command-not-run · built `release` gets the BUMPED manifests not HEAD's); hermetic. *(Out of `INCLUDE_GLOBS`.)*
...
- `tests/test_session_advisor.py` — tests for the SessionStart advisor: HARD invariants (silent path emits neither key, `MAX_LINE_CHARS` cap, fail-safe exit 0), the recommendation priority order, the RETURN-2/3/6 branches; tmp_path + stubbed `git log`. *(Out of `INCLUDE_GLOBS`.)*
- `tests/test_shipped_condensation_trigger.py` — regression scan (plan 0038 S1): reuses `build_release`'s ship classifier and asserts NO shipped file points an adopter at the orphaned "doc-budget WARN fires" condensation trigger (that gate is stripped from the release); pure `find_orphaned_triggers` core + both-direction pattern pins. *(Out of `INCLUDE_GLOBS`.)*
