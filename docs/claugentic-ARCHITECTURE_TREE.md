...
- `scripts/build_release.py` — release builder: `DEV_ONLY_PATH_CLASSES` = authored `path -> recreate-class` manifest (six classes); `classify`/`is_dev_only`/`recreate_class` derive from it; `--apply` rebuilds local `release` behind fail-loud preconditions (`_missing_upstream_commits` · `_version_increase_error` · `_dropped_shipped_paths`) then `_validate_built_tree` scans the stripped worktree pre-commit (P0-2, S5); `--dry-run` prints the split.
...
- `tests/test_build_release.py` — release-builder tests: `TestClassify`/`TestManifestMigration` pin the ship-vs-strip split + six-class no-op; `TestBaseAncestryGuard` pins `_missing_upstream_commits` (incl. non-merge broadening); the `Semver`/`VersionIncrease`/`LatestReleaseTag`/`ReadManifestVersion` classes pin the version-increase guard; `TestMechanizedDropCheck` the subset drop-check; hermetic via mocked `_git`. *(Out of `INCLUDE_GLOBS`.)*
