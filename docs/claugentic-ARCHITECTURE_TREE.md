...
- `scripts/claugentic-session-advisor.py` — open to change the SessionStart advisor (not a gate): derives ONE "where am I / what's next" line + two user-facing currency clauses (stamped-docs-behind-plugin skew · landed/cold-plan count, `COLD_DAYS`); agent-facing `additionalContext` only on the resume branch and never the clauses; `CLAUDE_HARNESS_ADVISOR=off` mutes; fail-safe exit 0.
...
- `tests/test_session_advisor.py` — tests for the SessionStart advisor: HARD invariants (silent path emits neither key, `MAX_LINE_CHARS` cap, fail-safe exit 0), the recommendation priority order, the RETURN branches, the currency nudges (skew · landed/cold · clause composition) + the audience-split pin keeping clauses out of `additionalContext`. *(Out of `INCLUDE_GLOBS`.)*
