...
- `.claude/agents/product-designer.md` — product/UX lens, two modes: Discover (Stage-1) surfaces user/JTBD/flows/states; Elevate (spec mode) critiques a draft (forcing functions + craft/feel signature-moment test). Applies `product-ux`, persists to PRODUCT.md.
- `.claude/agents/lens-reviewer.md` — applies ONE standards module (the lens), four modes: Verify-diff (Stage-7), Audit-scope (passed `depth`), Plan-design (Stage-2b), Whole-scope (the `thorough` cross-cutting red-team sweep). Per-lens fan-out; read-only, per-finding results.
...
- `.githooks/pre-commit` — open when changing the commit gate: wired via `core.hooksPath=.githooks`, `run_gate` runs each gate `--staged` once per `git commit` (stdout captured = silent clean pass, stderr flows through, exit 1 aborts); a failed git or a failed `"$PY" -c ""` probe skips loudly and passes. Template twin in `skills/init/SKILL.md` (parity-pinned).
...
- `engine/build-item.js` — open to change the build-to-green engine: iterates one approved item implement (implementer) → gates (`gatesGreen`, never fail-open) → Verify → QA → fix until green/cap. NEVER touches git; spawns no judge.
...
- `scripts/check_doc_budgets.py` — open to change the ledger byte-budget MECHANICS (caps are data: `.claude/claugentic-doc-budgets.json`): absent config = no-op, malformed = fail loud, `*`-key = glob, `reportOnly` = graced breach, ≥90% WARN (stderr — the advisory stream the pre-commit wrapper lets through; verdict stays stdout), independent reads. Not hook-wired.
...
- `tests/test_check_doc_budgets.py` — tests for the ledger byte-budget gate (under/at/over, missing/unreadable, independent reads, exit codes) + the config boundary: absent-vs-malformed, glob-by-key, dead-glob skip, subdir WARN, reportOnly, WARN-to-stderr stream contract. *(Out of `INCLUDE_GLOBS`.)*
- `tests/test_precommit_wrapper.py` — runs `.githooks/pre-commit` through real `sh` in a scratch repo (fake gate): missing/stub interpreter + broken git pass loudly, stdout captured vs stderr through, red aborts; plus the init-template parity pin and the husky marker-idempotency rule. *(Out of `INCLUDE_GLOBS`.)*
