...
- `tests/workflows/audit.test.mjs` — `node --test` tests for `engine/audit.js`'s pure helpers: `enumerateCells` (interleaved order + resume-determinism + blindspot strictly-last), `lensCoverage`/`renderLensCoverage` (ran-clean vs never-ran), `applyPrune` (`missing-test-baseline` never-pruned), `buildVerifierInput` independence, the fence renderer, the criteria seam. *(Out of `INCLUDE_GLOBS`.)*
