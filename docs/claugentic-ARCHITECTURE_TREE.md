...
- `tests/test_precommit_wrapper.py` — runs `.githooks/pre-commit` through real `sh` in a scratch repo (fake gate): stub-beside-working Python still gates, absent Python/gate script skip loudly, broken git skips silently, stdout captured vs stderr through, red aborts; plus the init-template parity pin and the husky chain rules. *(Out of `INCLUDE_GLOBS`.)*
...
- `tests/test_shipped_condensation_trigger.py` — inverted regression scan (0038 S1 → 0041 S6): now that the budget gate ships, asserts NO shipped file claims its WARN is unreachable, nor caveats the gate away with a `check_shipped_content` marker. *(Out of `INCLUDE_GLOBS`.)*
