...
- `docs/diagrams/*.{excalidraw,png}` — README diagrams (Excalidraw source + PNG); they **ship** with the plugin (default-include; `.png` skipped from `check_shipped_content`'s text scan, `.excalidraw` scanned as JSON): `harness-usage-flow` (the *Commands* command-map) and `harness-journey` (the *Under the hood* pipeline — four beats, gates, charter, Stage-9 loop).
