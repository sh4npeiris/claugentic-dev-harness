...
- **`product-critic`→mode CONTRADICTS a recorded DECISION:** `product-critic.md:8` asserts "SRP-separate from the designer, never a mode of it (DECISIONS → roles are SRP-separate)." This merge reverses that — **record a superseding DECISIONS entry** (and confirm the user accepts the SRP reversal; the first-principles pass already CONFIRMED the demotion, so this is bookkeeping, but the recorded-decision reversal must be explicit, honesty rule).
