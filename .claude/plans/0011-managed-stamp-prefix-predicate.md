...
Under the strict predicate, an **old-format-but-genuine** managed file fails leg 2 → classified `USER_FILE` → **skipped → never refreshed**. So `/init` would silently fail to refresh exactly the old adopters the version-aware feature exists to serve. **an adopter hit this on a real 0.1.1→0.1.15 refresh** (we ruled it manually).
