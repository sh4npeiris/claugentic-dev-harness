...
2. **Measured consequence.** On a mature repo with its own tree, that blocking gate full-scans an *incompatible* tree. Exact-token coverage simulation (2026-06-16): 6/7 neesh repos (`neeshinc, ConnectBase, DocBase, FlowBase, WatchBase, neeshOS`) score **0%** because their trees are **ASCII directory diagrams inside ` ``` `-fenced blocks**, which the gate's `_strip_fenced_blocks` strips → the gate flags **100%** of source files MISSING → every commit blocked. AskBase scores 95% (prior-init backtick tree). data-insights-hub survived only because it was *hand-grafted* with the gate off (no hooks in its `settings.json`) — never a real `init` run. The suite rollout *is* the first real `init` on mature-with-tree repos, so this surfaces now.
...
- **Keep mine, gate off** → leave the tree untouched; wire **neither** hook; set `INCLUDE_GLOBS = []` so a manual run can't false-flag. (This is data-insights-hub's circumstantial state, now an explicit choice.)
...
- **Evidence:** coverage sims — data-insights-hub 287/1455 (20%); neesh 0% ×6 (ASCII-in-fences) + AskBase 95%. data-insights-hub `settings.json` has **no** hooks (hand-grafted gate-off); its `.claude/rules/rules.md` confirms its tree was hand-maintained, no generator, no gate (model-upheld + review).
...
- **Backward-compat on re-`init`** (data-insights-hub @0.1.34, neesh prep branches) → re-`init` detects the existing tree and **prompts**; it **never auto-removes** a previously-wired hook (flag-and-recommend) — no silent settings churn.
...
- **In plain English (the approval gate):**
  ...
  - **What "done" means for you:** running `init` on any neesh repo no longer breaks commits — you pick "keep mine, gate off" (or "replace") and the harness wires accordingly. Re-running `init` changes nothing — no re-prompt, no git diff.
...
- **Skeleton format — the regression guard** _(reviewer acceptance criterion):_ the skeleton uses markdown headings + `- \`path\`` lines and **never** ` ``` `-fenced blocks (a fence is stripped by `_strip_fenced_blocks:129-150` and would desync the very pairing that caused the 6×0% neesh measurement).
...
- **Tests / verification** (SKILL prose + managed content → dogfooding + reviewers, no script logic change):
  - **Dogfood** on a throwaway copy/branch of a mature-with-tree neesh repo: (1) Replace → skeleton lists all in-scope paths, gate green, both hooks; (2) Keep-gate-off → no hooks, `INCLUDE_GLOBS=[]`, tree untouched; (3) **re-run = zero git diff + no re-prompt**; (4) skeleton has **no fenced blocks**.
