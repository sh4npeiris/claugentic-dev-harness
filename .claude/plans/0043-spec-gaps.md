...
- [x] **[PS-1] The per-line length limit on the code map is enforced but never explained** · `bug`
...
- [x] **[PS-2] Gap mode never tells you which parts of your spec the code does deliver** · `feature`
  - **LANDED.** Gap FIND now validates against `GAP_LENS_SCHEMA` (required `criterionVerdict`), and a new pure `criterionCoverage` + `renderCriterionCoverage` pair renders a per-criterion met/partial/missing/not-checked line in the fence's coverage slot — the verdict FOLDED against surviving evidence, attributed by the now-unioned `modules` (never first-wins `sourceModule`), `parseCellKey` out of the path. Pinned in `tests/workflows/audit.test.mjs` — grep `criterionCoverage` there for the list, including the dedup-collision and pipe-in-id regressions and the partial-vs-missing wording split (a locator, not a count).
...
- [x] **[PS-2] Spec-gap findings are pruned by a filter written for engineering audits** · `bug`
  - **LANDED at all three sites.** `synthesizer-gate` Mode 3 gained the gap variant (no YAGNI; cut only exact duplicates and criterion-less findings, every reason naming its criterion id; never a test-baseline item), `buildSynthesisPrompt(…, isGap)` branches exactly those two clauses and is passed `isGap` at BOTH call sites incl. the retry with the baseline injection `!isGap`-guarded, and the audit + product prose fallbacks quote the variant.
...
- [x] **[PS-2] Gap mode spawns the reviewer agent in a mode that agent's own contract does not define** · `bug`
...
- [x] **[PS-3] A run that ran out of budget mid-double-check still reports itself as complete** · `bug`
...
- [x] **[PS-4] The build engine's refusal message points at a mode that no longer exists** · `bug`
...
- [x] **[PS-4] "Not green - here is the residual" can be followed by an empty residual** · `bug`
...
- [x] **[PS-4] The build engine claims it never touches git, then tells the implementer to branch and commit** · `bug`
...
- [x] **[PS-4] The canned refusal about missing tests cannot describe its most common cause** · `bug`
...
- [x] **[PS-5] The cross-model claim ignores the reviewers that actually threw findings out** · `bug`
...
- [x] **[PS-5] Only the audit run has to tell you its cross-model outcome; verify, QA and build do not** · `feature`
...
- [x] **[PS-7] The over-budget alarm sends people to shave the wrong material** · `bug`
...
- [x] **[PS-7] The over-budget message never names the command that fixes it** · `bug`
...
- [x] **[PS-2] The harness's own ROADMAP says it has no product spec, while the spec sits next to it** · `bug`
...
- [x] **[PS-3] The summary calls disproved findings "couldn't be confirmed", which is what it calls the ones it kept** · `bug`
