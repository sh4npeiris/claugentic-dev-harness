...
- **Status:** APPROVED (Slice 1 only) — Slice 2/migration DROPPED. No backward-compat: the harness has been used on one project (an adopter repo), aligned by hand. Implementing.
...
- Existing adopters (an adopter repo, adopter) migrate cleanly on re-init — old un-prefixed harness-stamped copies are renamed, **user files never touched**.
