...
- **Resumable from:** S1 ✅ + S3 ✅ + S4a ✅ + S4b ✅ + S5 ✅ landed. **All 0024 slices complete** (S2 own-repo done; adopter-wiring folded into S4a). At close: dispose the S2 adopter-`init`-wiring remainder per the new disposition model (it landed in S4a, so it is **done**); the plan then completes. **doctor + unified finder pipeline → plan `0025`.**
...
- [x] **S1 — Tree condensation + CLAUDE.md read-economy wording.** ✅ LANDED — tree 30,780 B → **21,842 B** (≤27k target), DECISIONS 34,510 B (≤60k), CLAUDE 3,976 B (≤6k); 87/87 entries kept; gates+tests green. **Build FIRST** (reclaims DECISIONS headroom before per-slice bookkeeping lines land — review #7). **Measurable target (review #3):** tree ~30,780 B → **≤ 27,000 B**; DECISIONS stays **≤ 60,000 B** after eviction (doc-budget gate confirms; 33,770 B now).
...
- [x] **S3 — INVARIANTS budget + advisor off-switch & audience-split.** ✅ LANDED — off-switch (`enabled` flag, `build_output` stays pure), nuanced split (additionalContext+disclaimer on resume only; systemMessage-only nudges), INVARIANTS 20k budget + both-band tests; 175 pytest + 344 node green; honesty-reviewer **CLEAN** (RETURN-6 preserved). Two scripts + two test updates; independent. **Folded decisions:** (a) **off-switch** `CLAUDE_HARNESS_ADVISOR=off` → `build_output` returns `{}` (fail-safe to silent). (b) **audience-split = NUANCED (review #1):** keep `additionalContext` (with the existing RETURN-6 `ADVISORY_PREFIX` disclaimer) ONLY for the in-flight-plan **resume** branch (`recommend_next` priority 1 — a genuine next-action the agent should see); emit **`systemMessage`-only** for the three promotional nudges (open-backlog / PARTIAL-rerun / no-product-spec — "work the user didn't ask for"). Honors problem #5 WITHOUT regressing RETURN-6; update `tests/test_advisor.py` to assert the per-branch split. (c) **INVARIANTS budget = 20,000 B (review #2)** justified: an *accreting* ledger (sibling to DECISIONS) whose only growth-bound is this gate — budgeted because it accretes, even though read-on-demand not auto-loaded; test BOTH WARN (≥90%) + breach (≥100%) bands. **Trust surface → convene `honesty-reviewer` at Verify.**
- [x] **S4a — Shared-adopter pre-commit wiring (the S2 remainder).** ✅ LANDED — init step-5b rewritten to wire `.githooks/pre-commit` + `core.hooksPath .githooks` (gate-ON shared; never-clobber preserved on every path incl. existing-hooksPath conflict = report+don't-clobber); `--hook`/`--hook-write` modes + helpers + 17 dead-mode tests removed (main() now `--staged`/default, exit 1 only, no weakened survivors); 158 pytest + 344 node green; architect-reviewer **PASS**. `init` step-5b shared branch: write `.githooks/pre-commit` + `git config core.hooksPath .githooks`; **retire the now-unused `--hook`/`--hook-write` modes** + their helpers (`_check_written_file`/`_stop_hook_active_from_stdin`/`_written_path_from_stdin`) + tests (review #4 — DECIDED: REMOVE in this same slice, since init no longer wires them; removing earlier would strand init). Lands complete — init shared-branch text + dead-mode removal, test-guarded.
- [x] **S4b — `init` solo/local-only mode.** ✅ LANDED — mode trigger (ask, default Shared, recorded) + 4 solo divergences (`.git/info/exclude` · `.git/hooks/pre-commit` · skip 5c · `CLAUDE.local.md`) + `git check-ignore` fail-loud guard; never-track invariant verified in a scratch repo; architect-reviewer **PASS** after fixing an honesty over-claim (`CLAUDE.local.md` is NOT inherently gitignored — kept local via `.git/info/exclude`) + trimming the tree entry. Depends on S2 + S4a (review #5 split). The four diverging steps (managed docs → `.git/info/exclude`; hooks → `settings.local.json` + `.git/hooks/pre-commit`; skip 5c; CLAUDE anchor → `CLAUDE.local.md`) + the AskUserQuestion mode trigger + the recorded-mode line + the `git check-ignore` guard + the step-9 solo honesty/verification line.
- [x] **S5 — Plan lifecycle: disposition (blocked→defer→close) + scope-creep handling + Bugs capture + ROADMAP taxonomy.** ✅ LANDED (doc-only) — WORKFLOW Plan-file-lifecycle rewritten with the done/defer/reject disposition + in-flight (a)/(b) scope-split + the structurally-prevented-no-debt framing (model-upheld, not mechanical); build SKILL "Never invent scope" guardrail + Land step augmented; TEMPLATE Disposition note; ROADMAP `Bugs`/`Later`/`Rejected-findings` standing sections OUTSIDE both fences. Bugs-survives-re-audit proven STRUCTURALLY (sits outside `harness-audit:backlog` markers; `engine/audit.js` renders `renderedBacklog` only BETWEEN the markers + `skills/audit/SKILL.md` "replace only inside the fence"). Gates green (doc-budget, tree-check, pytest, node). **Folded (review #6):** ALSO edit `docs/claugentic-WORKFLOW.md`'s **Plan-file-lifecycle** section (`:168-170`, currently "on completion, remove it" — reconcile with the disposition model) — add to Affected-files. **Acceptance criterion (checkable):** the `Bugs` section sits OUTSIDE the regenerated `harness-audit`/`harness-product` fences so a re-audit never wipes it (dogfood: seed a `Bugs` entry, run `/audit`, confirm survival). **Harness-process change → diverse panel at Verify, not solo.**
