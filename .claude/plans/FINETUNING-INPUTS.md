...
- **the red-first `PreToolUse` hook** (charter/refactor characterization) was precisely spec-able but is **DECLINED (0035 rejected 2026-07-04)** — the harness does not force test-first; the discipline stays model-upheld by design.
...
> **⚠️ RELIABILITY FLAG (2026-07-04):** this "VERIFIED" pass was itself caught **OVER-STATING twice** — (1) the `/loop`-`/goal` "debunked/spins-up-its-own-harness" OVER-REFUTATION (it checked the /loop DOCS, not the BLOGS, and wrongly dismissed the real dynamic-workflow framing — see the corrected `/loop`-`/goal` bullet below); (2) the no-PostToolUse "keep-fast VALIDATED" OVER-CLAIM (below). Both surfaced only by **first-hand re-fetch of the actual source**. **Lesson: an over-refutation is as much an honesty error as an over-claim; do NOT trust this section's labels — re-verify any load-bearing literal against the real docs/blog before building on it** (blog ≠ docs are different altitudes).

A 6-cluster research fan-out WebFetched the real Claude Code docs/blog + checked the repo. Verdicts:

**Confirmed & worth building:** the model-tier menu (`best`/`fable`/`opus`/`sonnet`/`opusplan`/`[1m]` — Fable 5 = most-capable for *long multi-sitting* work, **NOT default**, access conditional → `best` auto-falls-back to Opus) · `opusplan` (opus-plan→sonnet-execute) · the **platform advisor** (`/advisor`, `advisorModel`, `--advisor`; escalates to a stronger model at decision points; **Anthropic-API-only, experimental v2.1.98+, beta header, subagents inherit**) · **no-PostToolUse CONSISTENT-WITH the docs** (⚠️ CORRECTED 2026-07-04, first-hand: the `hooks` doc DOES classify `PreToolUse`/`PostToolUse` as the highest cadence — "on every tool call" — but the explicit "keep fast" is stated only for **SessionStart**; the docs do **NOT** mandate avoiding per-tool hooks. The earlier "VALIDATED / keep-fast" was an **OVER-CLAIM** — built correctly-hedged in 0033) · **context-economy aligned** (official CLAUDE.md **~200-line target CONFIRMED first-hand**, skills-load-on-demand, subagent isolation; but the harness's per-ledger BYTE-caps are PROPRIETARY, not official) + prompt-cache dynamics (fresh subagent caches; **the "5-min TTL" number is UNVERIFIED — dropped in 0033**) · the **9-category skills taxonomy** (blog) · the **edge-skill adoptions** (`/debug` `/architecture` `/skill-creator` `/incident-response` `/deploy-checklist` are real bundled skills) · the **PreToolUse red-first hook was spec-able but DECLINED** (0035 rejected 2026-07-04 — model-upheld by design; the harness does not force test-first) · the **release "7-hand-lists-for-1-fact"** consolidation.

**Honesty corrections — do NOT build these as the dossier stated:**
- **`/loop`+`/goal` are scheduling/goal PRIMITIVES, not *themselves* the multi-agent orchestration.** `/loop` = three modes (fixed-interval · self-paced/dynamic · built-in maintenance); `/goal` = condition-based persistence. **⚠️ CORRECTION (2026-07-04, verified against the actual blogs — the earlier "drop turn/goal/time/proactive + spins-up-its-own-harness as WRONG" was an OVER-REFUTATION):** those framings are REAL, not debunked — the *loops* blog (`claude.com/blog/getting-started-with-loops`) defines the four loop TYPES (turn-based · goal-based · time-based · proactive) as a valid taxonomy, and the *dynamic-workflows* blog (`claude.com/blog/a-harness-for-every-task-...`) defines a **dynamic workflow** as *"Claude writing its own multi-agent harness on the fly"* (a JS file that spawns+coordinates subagents) — which is **exactly what the harness's `engine/*.js` + the Workflow tool ARE.** The prior research checked the /loop DOCS (command-level) and wrongly generalized to the BLOG framing (pattern-level). Correct position: /loop-/goal are primitives that **pair with** a dynamic workflow (run a repeatable workflow under `/loop` at intervals or `/goal` to a completion bar); the harness IS a dynamic-workflow harness. **Do NOT call the blog framing "debunked."** *(0032 landed with this over-refutation → corrected in WORKFLOW + DECISIONS 2026-07-04.)*
...
- **`0035`** — red-first PreToolUse test-gate — **REJECTED/DECLINED (2026-07-04): the harness does not force test-first; the discipline stays model-upheld by design.**
