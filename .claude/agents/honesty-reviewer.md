...
2. **For each, try to prove it over-claims.** Ask: does this word imply a mechanical guarantee? Is the wiring it implies actually present (use `Grep`/`Glob`/`Bash` read-only to look for the gate, the script, the hook)? Is the success claim scoped to what was reviewed? Apply signal-vs-noise: is this a *launder* or an *accurate* use?
