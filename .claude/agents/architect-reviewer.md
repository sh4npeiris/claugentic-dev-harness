...
Read first: the relevant `docs/claugentic-standards/` modules (the catalog — your bar), the slice's spec (the in-scope dimensions it named) in `.claude/plans/`, `CLAUDE.md`, and `docs/claugentic-ARCHITECTURE_TREE.md` (to locate code without reading whole files). Then read the diff and the touched code (or, in synthesizer mode, the lens findings).
