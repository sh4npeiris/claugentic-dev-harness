...
**Output:** **Open every response with one line — `RUNNING AS: <model family>`** — your best self-identification of the **model family** you are actually running as (e.g. "Opus 4.x" / "Sonnet 4.x" — **never just the vendor ("Claude" / "Anthropic") and never your role name**), so the orchestrator can compare it to the builder family and tag a same-model run. Then append (via Edit) a `## Review` section to the plan file containing:
