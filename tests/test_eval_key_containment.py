"""Containment pin for the eval answer keys — the exam ids stay inside their fixtures.
...
**TWO exams, two vocabularies, one mechanism.** The harness now sits a second one: the
BUILD-path eval (`eval/fixture-build/`), whose answer key names ten traps by id. Everything
above applies to it word for word, so it rides the same scan with its own vocabulary, its
own (much smaller) allowlist and its own do-not-revert note in the tree. The two are kept as
separate constants and separate tests on purpose: a leak of one is not a leak of the other,
and merging the allowlists would let an exemption earned by one exam quietly cover the
other. `tests/test_eval_trap_manifest.py` owns the trap table's shape, and this file imports
its row count for the same completeness reason.

One consequence worth naming, because it is doing real work: `eval/BUILD_BASELINE.md` is
**deliberately absent** from the trap allowlist. That file is the procedure a runner opens,
and the answer-free half of a day-one split; if an answer **naming a trap id** is ever
written into it, this scan is what says so. The paraphrase residual recorded above applies
here identically -- an answer written without an id stays green, and **that half is
model-upheld**.
...
from test_eval_trap_manifest import EXPECTED_ROWS as EXPECTED_TRAP_ROWS
...
TRAP_MANIFEST_REL = "eval/fixture-build/TRAP_MANIFEST.md"
TRAP_MANIFEST = REPO_ROOT / TRAP_MANIFEST_REL
TREE_TRAP_NOTE_ANCHOR = "Do not revert the `fixture-build/` entries below to trap-level detail."
...
# The build eval's own allowlist. Four entries, and each one carries the answers as its JOB:
# every other tracked file — including `eval/BUILD_BASELINE.md`, the procedure a runner opens,
# and both builder-visible artifacts inside the fixture itself — is scanned.
TRAP_ALLOWLIST: dict[str, str] = {
    TRAP_MANIFEST_REL: "the answer key — it defines the trap ids",
    # The probe registry is keyed BY trap id (`tests/test_eval_trap_manifest.py` asserts the
    # key set equals the manifest's), so the ids are the code's own vocabulary, not prose.
    "eval/fixture-build/checks/run_sweep.py": "the probe registry is keyed by trap id",
    # Results, per run: the per-trap tables ARE the measurement. This is the answer-bearing
    # half of the day-one procedure/entries split the sibling eval could not make.
    "eval/BUILD_ENTRIES.md": "records per-trap outcomes — the tables ARE the results",
    # The per-trap denylist that keeps the brief from coaching a remedy is keyed by id too.
    "tests/test_eval_trap_manifest.py": "the no-coaching denylist is keyed by trap id",
}
...
def _trap_ids() -> list[str]:
    """The trap-id vocabulary, derived from the build eval's answer key."""
    assert TRAP_MANIFEST.exists(), f"answer key missing: {TRAP_MANIFEST_REL}"
    text = TRAP_MANIFEST.read_text(encoding="utf-8")
    return [
        m.group(1)
        for line in text.splitlines()
        for m in [_ID_CELL_RE.match(line.strip())]
...
def _id_pattern(ids: list[str]) -> re.Pattern[bytes]:
...
def _files_naming_ids(root: Path, rels: list[str], pattern: re.Pattern[bytes]) -> list[str]:
    """The subset of `rels` (repo-relative) whose bytes contain one of `pattern`'s ids."""
...
def test_no_tracked_file_outside_the_allowlist_names_a_seed_id() -> None:
    ...
    scanned = [rel for rel in _tracked_files() if rel not in ALLOWLIST]
    offenders = _files_naming_ids(REPO_ROOT, scanned, pattern)
    assert offenders == [], (
...
def test_the_scan_fires_on_a_planted_id(tmp_path: Path) -> None:
    ...
    offenders = _files_naming_ids(
        tmp_path, ["docs/some-doc.md", "docs/innocent.md"], _id_pattern(_seed_ids())
    )
...
# ---------------------------------------------------------------------------
# The build eval's vocabulary -- same mechanism, its own corpus and allowlist
# ---------------------------------------------------------------------------
def test_trap_id_vocabulary_is_derived_and_complete() -> None:
    """A vocabulary that parsed short would narrow the scan silently."""
    ids = _trap_ids()
    assert ids, (
        "no trap ids parsed out of " + TRAP_MANIFEST_REL + " -- the answer-key table's "
        "shape changed and every trap-containment assertion below would pass vacuously. "
        "See tests/test_eval_trap_manifest.py, which owns that table's shape."
    )
    assert len(ids) == EXPECTED_TRAP_ROWS, (
        f"parsed {len(ids)} trap ids out of the answer key's {EXPECTED_TRAP_ROWS} rows -- a "
        "row whose id cell stopped matching _ID_CELL_RE silently DROPS that id from the "
        "containment vocabulary and it becomes freely leakable, with every test green."
    )


def test_the_two_vocabularies_do_not_overlap() -> None:
    """Two exams, two id sets. An overlap would make one exam's allowlist cover the other."""
    overlap = set(_seed_ids()) & set(_trap_ids())
    assert not overlap, (
        f"the two answer keys share id(s) {sorted(overlap)}. Each exam's allowlist would "
        "then exempt files for the OTHER exam's vocabulary -- rename one set."
    )


def test_no_tracked_file_outside_the_trap_allowlist_names_a_trap_id() -> None:
    """The pin: the build exam's answers are not republished anywhere an agent reads.

    The corpus deliberately includes the fixture's own builder-visible artifacts and
    `eval/BUILD_BASELINE.md`: a trap id appearing in the brief, the frozen plan slice or the
    procedure doc is the leak this scan exists for, not an exception to it.
    """
    pattern = _id_pattern(_trap_ids())
    scanned = [rel for rel in _tracked_files() if rel not in TRAP_ALLOWLIST]
    offenders = _files_naming_ids(REPO_ROOT, scanned, pattern)
    assert offenders == [], (
        "these tracked files name a build-eval trap id and are not allowed to: "
        f"{offenders}. A trap id in prose hands a builder the exam's answers without it "
        "ever opening the answer key. Describe what the fixture file IS, never what is "
        "planted in it; point at " + TRAP_MANIFEST_REL + " instead of quoting it."
    )


def test_every_trap_allowlist_entry_still_needs_its_exemption() -> None:
    """An exemption nothing uses is a hole nobody is watching."""
    pattern = _id_pattern(_trap_ids())
    tracked = set(_tracked_files())
    missing = sorted(rel for rel in TRAP_ALLOWLIST if rel not in tracked)
    assert not missing, f"allowlisted paths that are no longer tracked: {missing}"
    unused = sorted(
        rel for rel in TRAP_ALLOWLIST
        if not pattern.search((REPO_ROOT / rel).read_bytes())
    )
    assert not unused, (
        f"these files are allowlisted but no longer name any trap id: {unused}. Delete the "
        "entry -- a stale exemption silently widens the next leak's blast radius. "
        "Justifications: " + repr({rel: TRAP_ALLOWLIST[rel] for rel in unused})
    )


def test_the_trap_scan_spares_its_allowlist_and_only_its_allowlist(tmp_path: Path) -> None:
    """Non-vacuity, both directions, through the SAME expressions the live case uses."""
    planted = _trap_ids()[0]
    leak_rel = "docs/claugentic-ARCHITECTURE_TREE.md"
    corpus = [*TRAP_ALLOWLIST, leak_rel]
    for rel in corpus:
        f = tmp_path / rel
        f.parent.mkdir(parents=True, exist_ok=True)
        f.write_text(f"| {planted} | security | R3 | harm | check | [D] |", encoding="utf-8")
    pattern = _id_pattern(_trap_ids())
    assert all(pattern.search((tmp_path / rel).read_bytes()) for rel in corpus), (
        "a fixture does not carry the planted id -- this test would pass for the wrong reason."
    )
    offenders = _files_naming_ids(
        tmp_path, [rel for rel in corpus if rel not in TRAP_ALLOWLIST], pattern
    )
    assert offenders == [leak_rel], (
        f"expected only {leak_rel} reported (got {offenders}) -- the trap allowlist must "
        "spare its own members and nothing else."
    )


def test_the_tree_carries_the_do_not_revert_note_for_the_build_fixture() -> None:
    """The paraphrase half is model-upheld for BOTH exams -- pin both notes.

    A tree entry spelling out what a mutant directory changes, or what a probe catches,
    republishes the answer while the id scan above stays green. The note is the only thing
    standing in front of that, and a note nothing asserts is one a condensation pass drops.
    """
    tree = (REPO_ROOT / TREE_REL).read_text(encoding="utf-8")
    assert tree.count(TREE_TRAP_NOTE_ANCHOR) == 1, (
        f"{TREE_REL} must carry the build-fixture do-not-revert note exactly once (found "
        f"{tree.count(TREE_TRAP_NOTE_ANCHOR)}): {TREE_TRAP_NOTE_ANCHOR!r}."
    )
    para = tree.split(TREE_TRAP_NOTE_ANCHOR, 1)[1].split(chr(10) + chr(10), 1)[0]
    assert "tests/test_eval_key_containment.py" in para, (
        "the build-fixture do-not-revert note no longer points back at this pin -- a reader "
        "who reverts an entry is then told nothing about what goes red."
    )
