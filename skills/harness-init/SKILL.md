...
These are contracts `harness-init`'s own re-run idempotency **and** the later
`/harness-update` depend on — they are deliberate, not incidental:

1. **The managed-stamp** — every file `harness-init` *copies* gets a stamp on its **first
   line** so a managed copy is unmistakable and machine-parseable:
   - **Markdown:** `<!-- agentic-dev-harness@{VERSION} managed — do not edit; run /harness-update to refresh -->`
   - **Python:** `# agentic-dev-harness@{VERSION} managed — do not edit; run /harness-update to refresh`
   - `{VERSION}` is read from the plugin's `plugin.json` `version` field (e.g. `0.1.0`).
   - The greppable token is **`agentic-dev-harness@<semver>`**. Idempotency detects an
     already-copied managed file by the presence of `agentic-dev-harness@`; `/harness-update`
     (later) regexes the semver to compare versions. Do **not** vary this format.

2. **The CLAUDE.md `harness:managed` fence** — the harness section `harness-init` writes
   into the adopter's `CLAUDE.md` lives between exact HTML-comment markers:
   ```
   <!-- harness:managed:start -->
   …managed pointer block — refreshed by /harness-update…
   <!-- harness:managed:end -->
   ```
...
- **Per file: if the target path already exists, skip it and report "skipped (present)."**
  Detect an already-copied managed file by the **`agentic-dev-harness@` marker** on its
...
- **Stamp on copy**, not in the source. The source modules are pristine (editable
  upstream) and carry **no** stamp; `harness-init` adds the stamp as the copied file's
...
- **Reuse the layout detection from `/harness-audit` Phase 1 (Understand)** — its
...
- **Present *with* the fence →** **skip** (the re-run path) — the managed block is already
  ...
  `/harness-update`'s job, not init's — init is copy/seed-if-absent.)*
...
- The **plugin version** (`agentic-dev-harness@{VERSION}`) — a static token, not a date.
...
- A **Current scope** block, seeded once — a short, non-capping snapshot of which
  ...
  which `/harness-update` overwrites). Seed it from step 1's detected ecosystem (e.g. for
...
- **`docs/ROADMAP.md` absent →** create a seed (a one-line intro + an empty `## Later`
  human-owned section; `/harness-audit` later adds its `harness-audit:overview` /
...
- **Scan** for the adopter's own gates: lint/format (`eslint`, `.eslintrc*`, `prettier`),
  ...
  and CI (`.github/workflows`, `.gitlab-ci.yml`). **Reuse `/harness-audit` Phase 1's
...
Running `harness-init` twice on the same repo is **safe and a provable no-op**:
- Every **copy** is copy-if-absent (detected by the `agentic-dev-harness@` stamp).
...
- It does **not** refresh an already-copied managed file to a newer version — that is
  **`/harness-update`**'s job (it parses the `agentic-dev-harness@<semver>` stamp, compares
  versions, overwrites managed files, and re-merges hooks). `harness-init` is
  copy/seed-**if-absent** only.
- It does **not** audit your code or write a backlog — that is **`/harness-audit`**.
