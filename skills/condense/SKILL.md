...
- **Under the band (~80%)** → **clear any `reportOnly` grace on that entry, THEN** report the
  before/after byte figure and the new headroom. **This step is not optional and nothing mechanical
  does it** — `/doctor`, `init`, and the doc-budget gate script itself each say the flag is cleared
  here and nowhere else, so a pass that skips it leaves the ledger graced forever and its cap
  silently not enforcing. Once the file is genuinely under budget the grace has nothing left to
  excuse; leaving it is a gate switched off invisibly. Editing the entry is a config change — it rides the **same user-approved diff**, and
