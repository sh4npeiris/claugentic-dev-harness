...
judge roles — `claugentic-dev-harness:plan-reviewer` · `claugentic-dev-harness:honesty-reviewer` — cross-model per
`docs/claugentic-WORKFLOW.md` → Principles → "Convene the panel's judge roles with the `fable` model override".**
...
`claugentic-dev-harness:finding-verifier` — cross-model per `docs/claugentic-WORKFLOW.md` → Principles →
"Convene the panel's judge roles with the `fable` model override".** Run the
