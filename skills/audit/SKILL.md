...
### The rejected-findings memory  *("I disagree with this finding" — a dropped finding stays dropped)*

A user can judge a surfaced finding wrong. So a re-audit doesn't keep re-surfacing it, record the
user's dismissal in a **rejected-findings memory** that **mirrors product spec mode's
`<!-- product-critic:rejected-proposals -->` convention exactly** (same shape, reused — not a new
format): a **`<!-- harness-audit:rejected-findings -->`-fenced list**, **create-on-first-use** (write
the fence the first time the user drops a finding; absent until then), **one terse line per dropped
finding** (its plain title / finding-key, enough to recognize it), **never stamped**, and **user-owned**.
It lives in `docs/claugentic-ROADMAP.md` **OUTSIDE** the regenerate-don't-accumulate `harness-audit:backlog`
