...
- **Releasing? Follow `docs/RELEASE_CHECKLIST.md`.** ONE command up to a single human-gated push: `python scripts/build_release.py --apply --bump <version>` writes both manifests + builds + validates + STOPS and prints the gated `git tag && push --force-with-lease && push tag`. The tool never tags/pushes (an aborted run = zero side effects); the force-push + eval-drift/`BASELINE.md` stay model-upheld, and still run the `git range-diff` drop-check.
