...
- **Record decisions.** Append a dated one-liner to `docs/claugentic-DECISIONS.md` for any non-trivial choice, filed per its header's filing rule (consult it before re-litigating a past one). Out-of-scope ideas → `docs/claugentic-ROADMAP.md`, not into the current change. When a doc-budget WARN fires, condense that ledger inline — `/claugentic-dev-harness:condense` owns the procedure; git history is the archive.
