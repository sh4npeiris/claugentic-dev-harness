...
- **Releasing? Follow `docs/RELEASE_CHECKLIST.md` — CI publishes.** `python scripts/build_release.py --apply --bump <version>` PREPARES locally (both manifests + build + validate) and STOPS; commit the bump, then the one gated act is `git tag vX.Y.Z && git push origin main vX.Y.Z`. That tag fires `.github/workflows/release.yml`: every gate + `claude plugin validate --strict` re-run at the tagged commit, and only then does it push `release` + create the Release. The script never tags/pushes (an aborted prepare = no tag/push, but the rebuilt local `release` branch + bumped manifests remain). **A red run SPENDS the version: bump forward, never reuse a tag.** Eval-drift/`BASELINE.md` + `git range-diff` drop-check stay model-upheld.
